...
| 命令 | 用途 |
|------|------|
| **git clone / git pull** | 初始化和更新看板代码 |
| **node** | 运行看板应用服务 |
| **npm install / npm run** | 安装项目依赖和构建 |
| **npx** | 运行npm中的可执行工具 |
| **pm2 start / stop / restart** | 管理看板后台服务（可选） |

### 安全性说明

- 所有操作都在**本地机器**进行，无网络上传
- Git操作仅限于看板项目代码仓库
- PM2 用于本地进程管理，不涉及远程通信
- **数据永不离开你的电脑** — 所有 token/费用统计完全离线

首次运行 `/local-usage:init` 时会询问权限，之后自动处理，无再次提示。
